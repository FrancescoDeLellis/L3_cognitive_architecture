...
from L2_Wrapper import L2Wrapper
from util import produce_spike_signal_L0
...
if __name__ == "__main__":
    ...
    agent = L2Wrapper('model', save_path=path_to_data, n_participants=n_participants)

    phase_L0_vec = produce_spike_signal_L0(omega_L0 = 4, A_spike = 1.5, interv_spike = 3.0, length_spike = 0.5, time_end = 10, dt = 0.01) # Generate a synthetic L0 signal for testing. 
                                                                                                                                          # Comment line 41 during test with real people
    ...
    while True:
        try:
            ...
            if data == '' or not (ready_to_read or ready_to_write or exception):
                ...
                if iteration < len(phase_L0_vec): # Needed to choose what value of phase_L0_vec we have to give to the update_position function
                    phase_L0 = phase_L0_vec[iteration]
                else:
                    phase_L0 = phase_L0_vec[-1]
                    
                ic(position, delta_t)
                message = agent.update_position(position, delta_t, time, phase_L0=phase_L0_vec[iteration])  # Update position
