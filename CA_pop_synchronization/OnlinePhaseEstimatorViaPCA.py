...
class OnlinePhaseEstimatorViaPCA:
    ...
    def compute_PCA(self):
        ...
        if self.is_first_pca_computed:
            ...
            n_samples = min(len(self.trajectory),
                            self.n_samples_initial_direction_pca)  # Take the first n_samples of the trajectory
            ...
            for i in range(n_samples - 1):  # Compute the velocity in this n_samples time instants
                velocity[:, i] = (self.trajectory[i + 1] - self.trajectory[i]) / (
                            self.time_instants[i + 1] - self.time_instants[i])
            ...
            if self.start_from_extended_position == True:  # If all the people start from the arm extended position
                if np.dot(self.pca.components_[0], avg_vel) > 0:  self.pca.components_[0] = -1 * self.pca.components_[
                    0]  # Rotate the pca vector when PCA and avg_vel are concordant
