...
# ---------------------------------------------------------------- DQN BASED L3 AGENT
class L3_Agent:
    def __init__(self, n_nodes, omega_vals, coupling_strength, model_path, dt=0.01, virtual_agent=0):

        self.graph_nx = nx.complete_graph(n_nodes)    # TODO MC: I don't think it's needed to generate the whole graph.
        self.n_nodes = n_nodes
        self.omega_vals = omega_vals
        self.dt = dt
        self.coupling_vals = nx.to_numpy_array(self.graph_nx) * coupling_strength
        ...
        self.virtual_agent = virtual_agent  # TODO MC: if this is an id or an index, let the var name reflect it
        self.wrapping_domain = '-pi to pi'

    def l3_update(self, theta_old):
        sum_coupling = 0
        for ii in range(self.n_nodes):   # TODO MC: this could be vectorized and doesn't need to create a network
        ...
        theta_new = theta_old[self.virtual_agent] + self.dt * (self.omega_vals[self.virtual_agent] - sum_coupling)  # TODO MC: is it correct that sum_coupling appears with - ?
        return theta_new

    def observations_input(self, theta_obs):    # TODO MC: let us use verbs if possible for method names
